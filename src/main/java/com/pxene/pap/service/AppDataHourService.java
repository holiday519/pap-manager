...
public class AppDataHourService extends BaseService
{
    ...
    @Transactional
    public AppDataHourBean findAppDataHourById(Integer id)
    {
        AppDataHourModel appDataDayModel = appDataHourDao.selectByPrimaryKey(id);
        ...
        // 将DAO创建的新对象复制回传输对象中
        return modelMapper.map(appDataDayModel, AppDataHourBean.class);
    }

    
    @Transactional
    public List<AppDataHourBean> listAppDataHour()
    {
        AppDataHourModelExample example = new AppDataHourModelExample();
        
        List<AppDataHourModel> appDataDayModels = appDataHourDao.selectByExample(example);
        List<AppDataHourBean> appDataDayList = new ArrayList<AppDataHourBean>();
        ...
        else
        {
            ...
            for (AppDataHourModel appDataDayModel : appDataDayModels)
            {
                appDataDayList.add(modelMapper.map(appDataDayModel, AppDataHourBean.class));
            }
        }
