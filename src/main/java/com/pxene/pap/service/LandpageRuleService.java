...
@Service
public class LandpageRuleService extends BaseService {
	...
	public void saveLandpageRule(RuleBean ruleBean) throws Exception {
		LandpageRuleModel model = modelMapper.map(ruleBean, LandpageRuleModel.class);
		...
		try {
			...
			//添加关联关系
			addCampaignAndRule(ruleBean, StatusConstant.CAMPAIGN_RULE_TYPE_LANDPAGE);
			landpageRuleDao.insertSelective(model);
	...
	public void deleteLandpageRule(String id) throws Exception {
		LandpageRuleModel modelInDB = landpageRuleDao.selectByPrimaryKey(id);
		if (modelInDB == null) {
		...
		if (models.size() > 0) {
			...
			landpageRuleDao.deleteByPrimaryKey(id);
		}
		
	}
	
	public void updateLandpageRule(String id, RuleBean ruleBean) throws Exception {
		LandpageRuleModel modelInDB = landpageRuleDao.selectByPrimaryKey(id);
		if (modelInDB == null) {
			throw new ResourceNotFoundException();
		}
		
		LandpageRuleModel ruleModel = modelMapper.map(ruleBean, LandpageRuleModel.class);
		//删除所有关联关系
		...
		try {
			// 修改规则信息
			landpageRuleDao.updateByPrimaryKeySelective(ruleModel);
		} catch (DuplicateKeyException exception) {
			throw new DuplicateEntityException();
		}
	}
	
	public RuleBean findLandpageRuleById(String id) throws Exception {
		LandpageRuleModel landpageRuleModel = landpageRuleDao.selectByPrimaryKey(id);
		if(landpageRuleModel == null){
			throw new ResourceNotFoundException();
		}
		RuleBean ruleBean = modelMapper.map(landpageRuleModel, RuleBean.class);
		RuleBean bean = getParamForRule(ruleBean);
		
		return bean;
	}
	
	public List<RuleBean> listLandpageRule(String name) throws Exception {
		LandpageRuleModelExample example = new LandpageRuleModelExample();
		
		if (!StringUtils.isEmpty(name)) {
			example.createCriteria().andNameLike("%" + name + "%");
		}
		List<LandpageRuleModel> rules = landpageRuleDao.selectByExample(example);
		...
		for (LandpageRuleModel model : rules) {
	 ...
	 * @throws Exception
	 */
	public void updateLanpageRuleStatus(String id, Map<String, String> map) throws Exception {
