...
public class TimeRuleController {
	...
	@RequestMapping(value = "/rule/time/status/{id}", method = RequestMethod.PUT)
	@ResponseBody
	public void updateTimeRuleStatus(@PathVariable String id, @RequestBody Map<String, String> map, HttpServletResponse response) throws Exception {
		timeRuleService.updateTimeRuleStatus(id, map);
		response.setStatus(HttpStatus.NO_CONTENT.value());	
	}
    ...
    @ResponseBody
	public String listTimeRules(@RequestParam(required = false) String name, @RequestParam(required = false) Integer pageNo, @RequestParam(required = false) Integer pageSize, HttpServletResponse response) throws Exception {
